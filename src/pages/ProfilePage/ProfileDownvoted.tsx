...
export default function ProfileDownvoted() {
  const { id } = useParams();
  ...
  const getDownvotedPostsQuery = useQuery({
    queryKey: ['fetch/downvotedPosts', id, query],
    queryFn: () => axios.get<IResponseData<Post[]>>(`/v1/posts/downvoted`, { params: query }),
  ...
  const posts = getDownvotedPostsQuery.data?.data.data;
  return (
    <>
      ...
      <div className="w-full">
        {getDownvotedPostsQuery.isLoading ? (
