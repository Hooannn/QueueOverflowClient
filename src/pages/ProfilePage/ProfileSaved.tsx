...
export default function ProfileSaved() {
  ...
  const getSavedPostsQuery = useQuery({
    queryKey: ['fetch/savedPosts', id, query],
    queryFn: () => axios.get<IResponseData<Post[]>>(`/v1/saved_posts`, { params: query }),
  ...
  const posts = getSavedPostsQuery.data?.data.data;
  return (
    <>
